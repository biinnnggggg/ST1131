...
We observe that \code{resale_price} quantitative which is necessary for regression modelling and the distribution is right-skewed. There is weak or no correlation between \newline\code{lease_commence_date} and \code{resale_price} as shown by the low correlation of 0.370 so we will not consider \code{lease_commence_date}as a regressor. There is a strong linear correlation between \code{floor_area_sqm} and \code{resale_price}.
\begin{verbatim}
    > head(street_name)
...
where \code{log(resale_price)} is the log-transformed response variable, the resale price of HDBs, and \code{floor_area_sqm}  is the explanatory variable representing the floor area of the HDB flat in square meters ($m^2$).
